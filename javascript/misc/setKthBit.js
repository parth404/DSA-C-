...
// Your task is to complete the function setKthBit() which takes two integer N and K as input
// parameter and returns an integer after setting the K'th bit in N.
...
function setKthBit(N, K) {
  // Create a bitmask with the Kth bit set (1 << K)
  // Then perform a bitwise OR operation to set the Kth bit in N
  return N | (1 << K);
}

// Example usage:
const N = 5; // Binary: 101
const K = 1; // Set the 2nd bit (from the right)
const result = setKthBit(N, K);
console.log(result); // Output will be 7 (binary: 111)
