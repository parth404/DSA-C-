...
// raises a giant sawblade to that height and cuts off all tree parts higher than H
// (of course, trees not higher than H meters remain intact). Mirko then takes the parts that were
// cut off. For example, if the tree row contains trees with heights of 20, 15, 10, and 17 metres,
// and Mirko raises his sawblade to 15 metres, the remaining tree heights after cutting will be
// 15, 15, 10, and 15 metres, respectively, while Mirko will take 5 metres off the first tree and
...
long long int sol(vector<long long int> trees, long long int m)
{
...
int main()
{
    long long int n, m; // number of trees and minimum wood required
    cin >> n >> m;
    vector<long long int> trees; // array of trees
    while (n--)
    {
        long long int h; // height of trees
        cin >> h;
        trees.push_back(h);
    }

    cout << sol(trees, m) << endl;
